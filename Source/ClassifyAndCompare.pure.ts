import { Allow, Ask, Bind, type Checker, Error, Pure } from "./Checker.pure.ts"
...
const classifyAndCompare = <T extends PartialOrderKey>(
	...
	Bind(Ask, ([xPath, yPath]) => {
		const xClass = classify(xPath)
		const yClass = classify(yPath)

		if (xClass === null || yClass === null) return Pure(void 0)
		...
		switch (QueryPartialOrder(po, xClass, yClass)) {
			...
			case "?":
				...
				return Error(`ClassifyAndCompare: can not depend on ${yPath}`)
		}
	})
 ...
 * `classRules`, then the returned checker behaves as `Pure(void 0)` (i.e.,
