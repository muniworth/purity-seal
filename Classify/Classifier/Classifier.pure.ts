...
export const Bind_ = <A, B>(m: Classifier<A>, f: (t: A) => Classifier<B>) =>
	(ext: ext) => Option.Bind(m(ext), t => f(t)(ext))
export const Bind: {
	<A, B>(m: Classifier<A>, f: (t: A) => Classifier<B>): Classifier<B>
	<A, B>(f: (t: A) => Classifier<B>): (m: Classifier<A>) => Classifier<B>
} = CurryRev(Bind_)
...
export const Catch_ = <A, B>(m1: Classifier<A>, m2: Classifier<B>): Classifier<A|B> =>
	(ext: ext) => Option.Catch_(m1(ext), () => m2(ext))
/** Recover if a classifier fails to match an extension. */
export const Catch: {
	<A, B>(m1: Classifier<A>, m2: Classifier<B>): Classifier<A|B>
	<A, B>(m2: Classifier<B>): (m1: Classifier<A>) => Classifier<A|B>
} = CurryRev(Catch_)
...
export const AsksWhen_ = <A extends string>(p: (ext: ext) => boolean, m: Classifier<A>): Classifier<A> =>
	Reader.Bind_(Reader.Asks(p), b => b ? m : None())

/** Use a classifier if the filepath satisfies a predicate. */
export const AsksWhen: {
	<A extends string>(p: (ext: ext) => boolean, m: Classifier<A>): Classifier<A>
	<A extends string>(m: Classifier<A>): (p: (ext: ext) => boolean) => Classifier<A>
} = CurryRev(AsksWhen_)

export const SetWhen_ = <A extends string>(p: (ext: ext) => boolean, f: (ext: ext) => A): Classifier<A> =>
	AsksWhen_(p, Flow(f, Option.Some))
/** Set the file classification if the filepath satisfies a predicate. */
export const SetWhen: {
	<A extends string>(p: (ext: ext) => boolean, f: (ext: ext) => A): Classifier<A>
	<A extends string>(f: (ext: ext) => A): (p: (ext: ext) => boolean) => Classifier<A>
} = CurryRev(SetWhen_)
